...
if ($_POST['department'] == 'department') {
  ...
  $objPHPExcel->getActiveSheet()->getCell('A1')->setValue("Postal Received");
 ...
 /*  $objPHPExcel->getActiveSheet()->getStyle(
  ...
  $objPHPExcel->getActiveSheet()->setCellValue('A2', 'Letter Recieved Date');
  $objPHPExcel->getActiveSheet()->setCellValue('B2', 'Purpose');
  $objPHPExcel->getActiveSheet()->setCellValue('C2', 'Client Name');
  $objPHPExcel->getActiveSheet()->setCellValue('D2', 'Application ID');
  $objPHPExcel->getActiveSheet()->setCellValue('E2', 'From ');
  $objPHPExcel->getActiveSheet()->setCellValue('F2', 'Document details');
  $objPHPExcel->getActiveSheet()->setCellValue('G2', 'Consignment No');
  $objPHPExcel->getActiveSheet()->setCellValue('H2', 'Consignment Date');
  $objPHPExcel->getActiveSheet()->setCellValue('I2', 'Receiver\'s Name');
  $objPHPExcel->getActiveSheet()->setCellValue('J2', 'Receiver\'s Phone');
  $objPHPExcel->getActiveSheet()->setCellValue('K2', 'Receiver\'s post');
  $objPHPExcel->getActiveSheet()->setCellValue('L2', 'Receiver Remark');
  $objPHPExcel->getActiveSheet()->setCellValue('M2', 'Revert status');
...
//$objPHPExcel->getActiveSheet()->getColumnDimension('A2:N2')->setAutoSize(true);
  ...
  $sql2 = "SELECT * FROM `recicved_letter` WHERE 1=1 ".$role_item."".$srcitem."  ORDER BY id DESC";
...
// echo $sql2; die(' sda');
  ...
  if (mysqli_num_rows($result2) > 0) {
	while ($row = mysqli_fetch_array($result2)) {
	  ...
	  $objPHPExcel->getActiveSheet()->setCellValue('A' . $rowCount, $row['letter_recieved_date']);
	  $objPHPExcel->getActiveSheet()->setCellValue('B' . $rowCount, $row['purpose']);
	  $objPHPExcel->getActiveSheet()->setCellValue('C' . $rowCount, $row['client_name']);
	  $objPHPExcel->getActiveSheet()->setCellValue('D' . $rowCount, $row['application_id']);
	  $objPHPExcel->getActiveSheet()->setCellValue('E' . $rowCount, $row['from_address']);
	  $objPHPExcel->getActiveSheet()->setCellValue('F' . $rowCount, $row['document_details']);
	  $objPHPExcel->getActiveSheet()->setCellValue('G' . $rowCount, $row['consignment_no']);
	  $objPHPExcel->getActiveSheet()->setCellValue('H' . $rowCount, $row['consignment_date']);
	  $objPHPExcel->getActiveSheet()->setCellValue('I' . $rowCount, $row['receivers_name']);
	  $objPHPExcel->getActiveSheet()->setCellValue('J' . $rowCount, $row['receivers_phone']);
	  $objPHPExcel->getActiveSheet()->setCellValue('K' . $rowCount, $row['receivers_post']);
	  $objPHPExcel->getActiveSheet()->setCellValue('L' . $rowCount, $row['receiver_remark']);
	  $objPHPExcel->getActiveSheet()->setCellValue('M' . $rowCount, $row['revert_status']);
...
$objPHPExcel->getActiveSheet()->getStyle('B3')->getFill()->getStartColor()->setARGB('FFFF0000'); */
  ...
  $file = "Postal Received-" .$dated. ".xlsx";
