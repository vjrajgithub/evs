...
$mm_id = $_GET['mm_id'];
if ($mm_id != '' && $mm_id != 0) {
  $query = "DELETE FROM `case_allocate` WHERE id='" . $mm_id . "'";
  // echo $query; die;
...
function get_user_name($mycon, $user_id){
  ...
  <div class="pcoded-inner-content">
    <div class="main-body">
      <div class="page-wrapper">
        ...
        <h5 class="page_header_top">Case Allocation</h5>
        <!--***** Tab_from advance search  start *******-->
        <div class="wrapper advance_search_excel">
          <div class="portlet-title hdr">
            <div class="caption">
              <button type="button" class="btn btn-success" onclick="reset();" data-toggle="modal" data-target="#exampleModal"> Add New <i class="fa fa-plus-circle" aria-hidden="true"></i>
              </button>
                      ...
                      <div class="table-container">
                        <table class="table table-striped table-bordered table-hover" id="sample_3">
                          ...
                          <tbody>
                            ...
                            $query = "SELECT ca.*, ca.id as case_allocation_id, ca.application_id as allocated_application_id,ca.user_id as allocated_user_id, ta.*,ta.client_name as client_code,ta.client_relationship_person_name as application_name,ta.case_id as case_id, ep.*  FROM case_allocate ca, tbl_application ta , employee_personal_info_tbl ep WHERE ca.application_id=ta.application_ref_id AND ta.application_ref_id = ep.application_id  order by ca.user_id";
                            // echo $query;
                            $result = mysqli_query($mycon, $query);
                            if (mysqli_num_rows($result) > 0) {
                              while ($allocate_data = mysqli_fetch_assoc($result)) {
                            ?>
                                <tr style="display: table-row;">

                                  <td><?php echo get_user_name($mycon, $allocate_data['allocated_user_id']); ?></td>
                                  <?php getTypeOfCheckName($mycon, explode(',', $allocate_data['allocate_type_of_check'])); ?>

                                  <td><?php echo $allocate_data['allocated_application_id']; ?></td>
                                  <td><?php echo $allocate_data['client_code']; ?></td>
                                  <td><?php echo $allocate_data['firstName'] . ' ' . $allocate_data['middleName'] . ' ' . $allocate_data['lastName']; ?></td>
                                  <td><?php echo $allocate_data['case_id'] ?></td>

                                  <td>
                                    <?php $applicant_name = $allocate_data['firstName'] . ' ' . $allocate_data['middleName'] . ' ' . $allocate_data['lastName']; ?>
                                    <a href="#" title="view"><i class="fa fa-eye" onclick="set_all_data_new('<?php echo get_user_name($mycon, $allocate_data['allocated_user_id']); ?>', '<?php echo $allocate_data['allocated_application_id']; ?>', '<?php echo $allocate_data['client_code']; ?>', '<?php echo $allocate_data['case_id']; ?>', '<?php  getTypeOfCheckName($mycon, explode(',', $allocate_data['allocate_type_of_check'])); ?>','<?php echo $applicant_name; ?>')" aria-hidden="true" data-toggle="modal" data-target="#mis_view_modal"></i></a>

                                    <!-- <a href="#" title="Edit"><i class="fa fa-pencil-square-o" aria-hidden="true" data-toggle="modal" data-target="#mis_edit_modal<?php echo $allocate_data['case_allocation_id'] ?>"></i>
                                  </a> -->
                                    <a href="case_allocation.php?mm_id=<?php echo $allocate_data['case_allocation_id']; ?>" title="Delete" onclick="return confirm('Are you sure?')"><i class="fa fa-trash-o" aria-hidden="true"></i> </a>
                                  </td>
                                </tr>
                            <?php }
                            } ?>
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header" style="border-bottom: 0; padding-bottom: 0;">
            <h5 class="modal-title" id="exampleModalLabel">Case Allocation Info</h5>
            ...
            <table class="table">
              <tbody>
                ...
                <tr>
                  <th>Application Id </th>
                  <td>:</td>
                  <td id="application_id_view"><?php echo $customer['customer_code'] ?></td>
                </tr>
                <tr>
                  <th>Client Code</th>
                  <td>:</td>
                  <td id="client_code_view"><?php echo $customer['state'] ?></td>
                </tr>
                <tr>
                  <th>Applicant Name</th>
                  <td>:</td>
                  <td id="applicant_name_view"><?php echo $customer['state'] ?></td>
                </tr>
                <tr>
                  <th>Case Id</th>
                  <td>:</td>
                  <td id="case_id_view" ><?php echo $customer['city'] ?></td>
                </tr>
                <tr>
                  <th>Type of Check</th>
                  <td>:</td>
                  <td id="type_of_check_view" ><?php echo $customer['city'] ?></td>
                </tr>
  ...
  <div class="modal-dialog" role="document" style="max-width: 620px;">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Add Case Allocation Data </h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <!-- <form id="allocata_case"> -->
        <input type="hidden" name="form_type" value="allocate_case">
        <div class="row">
          <div class="col-md-6">
          <div class="form-group">
              <label class="input_hdr" for="application_id"> Application Id &#9734; </label>
              <select chosen name="application_id" id="application_id" onchange="changed_application_id('application_id','type_of_check');" class="form-control">
                <!-- <option value="India"><?php echo 'India'; ?></option> -->
                <option value=""> Select </option>

                <?php
                $query5 = "SELECT * FROM tbl_application WHERE application_status = 3";
                $result5 = mysqli_query($mycon, $query5);
                if (mysqli_num_rows($result5) > 0) {
                  while ($application = mysqli_fetch_assoc($result5)) {
                ?>
            ...
            <div class="form-group">
              ...
              <select chosen name="type_of_check" id="type_of_check" multiple="multiple" class="form-control" style="height: 200px">
            ...
            <!-- <div class="form-group">
                ...
                <div>
                  <input type="text" id="application_id" name="application_id" required class="form-control">
                </div>
              </div> -->
            <!-- <div class="form-group">
                <label class="control-label">Type of Check &#9734;
                </label>
                <div>
          ...
          <div class="col-md-6">
            ...
            <!-- <div class="form-group">
              ...
              <div class="form-group">
              <label class="input_hdr" for="user_name"> User Name &#9734; </label>
              <select chosen name="user_name" id="user_name" class="form-control">
                <!-- <option value="India"><?php echo 'India'; ?></option> -->
                <option value=""> Select </option>

                <?php
                $query5 = "SELECT u.id as u_id, u.username as u_name, u.role as u_role, tr.description as role_desc  FROM users u, tbl_roles tr WHERE u.role = tr.roleId  AND u.role IN(25,26,27,30,31,32,33,34,35,36)";
                // echo $query5; die;
                $result5 = mysqli_query($mycon, $query5);
                if (mysqli_num_rows($result5) > 0) {
                  while ($user = mysqli_fetch_assoc($result5)) {
                ?>
                    <option value="<?php echo $user['u_id']; ?>"><?php echo $user['u_name'] ?>-<?php echo $user['u_role'] ?>-<?php echo $user['role_desc'] ?></option>
                <?php }
                } ?>
...
<div id="other_details" style="display: none;">
                <div class="form-group">
                  ...
                  <div>
                    <input type="text" id="case_rec_date" name="case_rec_date" data-required="1"  class="form-control">
                  </div>
                </div>
                <div class="form-group">
                  ...
                  <div>
                    <input type="text" id="client_code" name="client_code"  data-required="1" class="form-control">
                  </div>
                </div>
                ...
                <div class="form-group">
                  ...
                  <div>
                    <input type="text" id="case_id" name="case_id" data-required="1"   class="form-control">
                  </div>
                </div>
                <div class="form-group">
                  ...
                  <div>
                    <input type="text" id="applicant_name" name="applicant_name"   data-required="1" class="form-control">
                  </div>
                </div>              
            ...
            <div id="prother_app_details">
              <div class="col-md-6">
                <div class="form-group">
                  ...
                  <div>
                    <input type="Date" id="case_rec_date" name="case_rec_date" data-required="1" class="form-control">
                  </div>
                </div>
                <div class="form-group">
                  <label class="control-label"> Client Code
        ...
        <div class="modal-footer">
          ...
          <button type="" onclick="send_data_toserver()" class="btn btn-primary"> Allocate </button>
        </div>
...
<script>
  ...
  function changed_application_id(application_dd, type_check_dd) {
    ...
    xmlhttp.open("GET", "type_of_check_back.php?application_id=" + document.getElementById(application_dd).value, false);
    xmlhttp.send(null);
    // alert(xmlhttp.responseText);
    document.getElementById(type_check_dd).innerHTML = xmlhttp.responseText;
    
    $.post("get_rest_application_details.php?application_id=" + application_id, function(data) {
              var res = $.parseJSON(data);
            //   alert(res['case_id']+res['client_name']+"  "+res['allocated_case_rec_date'] );

            document.getElementById('case_rec_date').value = res['allocated_case_rec_date'];
            document.getElementById('client_code').value = res['client_name'];
            document.getElementById('case_id').value = res['case_id'];
            document.getElementById('applicant_name').value = res['applicant_full_name'];

            document.getElementById("case_rec_date").disabled = true;
...
function reset(){
			...
			document.getElementById('type_of_check').value = "";
			document.getElementById('application_id').value = "";
			document.getElementById('case_rec_date').value = "";
...
function set_all_data_new (user_id,  allocated_application_id, client_code, case_id, type_of_check_view, applicant_name) {
// alert(type_of_check_view);
document.getElementById('user_name_view').innerHTML = user_id;
document.getElementById('application_id_view').innerHTML = allocated_application_id;
document.getElementById('client_code_view').innerHTML = client_code;
document.getElementById('case_id_view').innerHTML =  case_id;
document.getElementById('type_of_check_view').innerHTML =  type_of_check_view;
document.getElementById('applicant_name_view').innerHTML =  applicant_name;
  ...
  function send_data_toserver() {
    ...
    var allocated_type_of_check = Array.from(document.getElementById("type_of_check").options).filter(option => option.selected).map(option => option.value);
    // alert(allocated_type_of_check);
    // var case_rec_date = document.getElementById('case_rec_date').value;
    ...
    } else if (allocated_type_of_check == "" || allocated_type_of_check == "0") {
      alert('Select  Type of check');
    } else {
      urll = "send_allocate_type_check_data_to_server.php?data=" + user_id + "~" + application_id + "~" + allocated_type_of_check;
      ...
      xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
          if (this.responseText == 1 || this.responseText == '1') {
            ...
            window.location = "case_allocation.php";
            // $.post("get_rest_application_details.php?application_id=" + application_id, function(data) {
            //   var res = $.parseJSON(data);
            // //   alert(res['case_id']+res['client_name']+"  "+res['allocated_case_rec_date'] );
            ...
            // // document.getElementById("case_id").style.display = "block";
            // // document.getElementById("applicant_name").style.display = "block";

            // });
            
            // $("#prother_app_details").load("get_rest_application_details.php?application_id=" + application_id);
