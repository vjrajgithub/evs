...
if ($_POST['department'] == 'department') {
  ...
  $objPHPExcel->getActiveSheet()->getCell('A1')->setValue("Postal Dispatch");
 ...
 /*  $objPHPExcel->getActiveSheet()->getStyle(
  ...
  $objPHPExcel->getActiveSheet()->getStyle('A1:N1')->applyFromArray($styleArray_header);
  ...
  $objPHPExcel->getActiveSheet()->setCellValue('A2', 'Letter Send Date');
  $objPHPExcel->getActiveSheet()->setCellValue('B2', 'Client Name');
  $objPHPExcel->getActiveSheet()->setCellValue('C2', 'Type of Checks');
  $objPHPExcel->getActiveSheet()->setCellValue('D2', 'Candidate Name');
  $objPHPExcel->getActiveSheet()->setCellValue('E2', 'Case ID');
  $objPHPExcel->getActiveSheet()->setCellValue('F2', 'Application ID');
  $objPHPExcel->getActiveSheet()->setCellValue('G2', 'To');
  $objPHPExcel->getActiveSheet()->setCellValue('H2', 'Document details');
  $objPHPExcel->getActiveSheet()->setCellValue('I2', 'Letter Sender Name');
  $objPHPExcel->getActiveSheet()->setCellValue('J2', 'Consignment No.');
  $objPHPExcel->getActiveSheet()->setCellValue('K2', 'Charges');
  $objPHPExcel->getActiveSheet()->setCellValue('L2', 'Consignment Date');
  $objPHPExcel->getActiveSheet()->setCellValue('M2', 'Sender Remark');
  $objPHPExcel->getActiveSheet()->setCellValue('N2', 'Created At');
 ...
 //$objPHPExcel->getDefaultStyle()->applyFromArray($styleArray);
  ...
  $objPHPExcel->getActiveSheet()->getStyle('A2:N2')->applyFromArray($styleArray1);
  $objPHPExcel->getActiveSheet()->getStyle('A2:N2')->applyFromArray($styleArray);
  $objPHPExcel->getActiveSheet()->getStyle('A2:N2')->applyFromArray($style_cell);
...
//$objPHPExcel->getActiveSheet()->getColumnDimension('A2:N2')->setAutoSize(true);
  ...
  $sql2 = "SELECT * FROM `send_letter` WHERE 1=1 ".$role_item."".$srcitem." ORDER BY id DESC";
...
// echo $sql2; die(' sda');
  ...
  if (mysqli_num_rows($result2) > 0) {
	while ($row = mysqli_fetch_array($result2)) {
      ...
      $typeofcheck= getTypeOfCheckName_excel($mycon, explode(',', $row['Type_of_Checks'])); 
        // $typeofcheck =    getTypeOfCheckName($mycon, explode(',', $row['Type_of_Checks']));

	  $objPHPExcel->getActiveSheet()->setCellValue('A' . $rowCount, $row['letter_send_date']);
	  $objPHPExcel->getActiveSheet()->setCellValue('B' . $rowCount, $row['client_name']);
	  $objPHPExcel->getActiveSheet()->setCellValue('C' . $rowCount, $typeofcheck);
	  $objPHPExcel->getActiveSheet()->setCellValue('D' . $rowCount, $row['candidate_name']);
	  $objPHPExcel->getActiveSheet()->setCellValue('E' . $rowCount, $row['case_id']);
	  $objPHPExcel->getActiveSheet()->setCellValue('F' . $rowCount, $row['application_id']);
	  $objPHPExcel->getActiveSheet()->setCellValue('G' . $rowCount, $row['to_address']);
	  $objPHPExcel->getActiveSheet()->setCellValue('H' . $rowCount, $row['document_details']);
	  $objPHPExcel->getActiveSheet()->setCellValue('I' . $rowCount, $row['letter_sender_name']);
	  $objPHPExcel->getActiveSheet()->setCellValue('J' . $rowCount, $row['consignment_no']);
	  $objPHPExcel->getActiveSheet()->setCellValue('K' . $rowCount, $row['postal_charges']);
	  $objPHPExcel->getActiveSheet()->setCellValue('L' . $rowCount, $row['consignment_date']);
	  $objPHPExcel->getActiveSheet()->setCellValue('M' . $rowCount, $row['sender_remark']);
	  $objPHPExcel->getActiveSheet()->setCellValue('N' . $rowCount, $row['created_at']);
...
$objPHPExcel->getActiveSheet()->getStyle('B3')->getFill()->getStartColor()->setARGB('FFFF0000'); */
  ...
  $file = "Postal Dispatch-" .$dated. ".xlsx";
