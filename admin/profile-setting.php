  ...
  if ($name && $username) {
	...
	//if ($password == $cpassword) {
	  ...
	  if (userExistsupdate($username) === TRUE) {
		...
		$sql = "UPDATE users SET username = '$username', name = '$name', contact = '$contact', email = '$email',  avatar = '$filename' WHERE id = '$edituser_id'";
	        ...
	        if ($result) {
		       ...
		       header("Location: profile-setting.php?msgprt=$msg123");
		   } else {
		     $msg = "Error";
		   }
	  }
							...
							<div class="card-block">
								...
								<form class="login-form" action="" method="POST" enctype="multipart/form-data">
									...
									<div>
										...
										<input type="text" class="form-control" style="max-width:50%"  name="name" placeholder="Full Name" autocomplete="off" value="<?php echo $userdata['name']; ?>" />
									</div>
									<br>
									...
									<div>
										...
										<?php $sql_query12 = "SELECT * FROM tbl_roles WHERE roleId = '".$userRole."'";
										   $result12 = mysqli_query($mycon, $sql_query12);
											 if (mysqli_num_rows($result12) > 0) { 
												while ($row12 = mysqli_fetch_assoc($result12)) { 
											    if($row12['description'] != ''){ $description = " - ".$row12['description']; } else { $description = ""; } ?>
											    <option value="<?php echo $row12['roleId']; ?>" <?php if($userdata['role'] == $row12['roleId']){ echo "selected"; } ?>><?php echo $row12['role'].$description; ?></option>
											 <?php }   }  ?>
										</select>
									</div>
									<br>
									...
									<div>
										...
										<input type="text" name="username" class="form-control" style="max-width:50%" placeholder="Username" autocomplete="off" value="<?php echo $userdata['username']; ?>" />
									</div>
									<br>

									<div>
										<label for="email">Email: </label>
										<input type="text" name="email" style="max-width:50%"  class="form-control" placeholder="Email" autocomplete="off" value="<?php echo $userdata['email']; ?>" />
									</div>
									<br>
									
									<div>
										<label for="contact">Contact No.: </label>
										<input type="text" name="contact" style="max-width:50%"  class="form-control" placeholder="Contact" autocomplete="off" value="<?php echo $userdata['contact']; ?>" />
									</div>
									<br>
									...
									<div>
										...
										<input type="hidden" name="updatedavatar" value="<?php echo $userdata['avatar']; ?>">  
										<img src="upload/user-profile/<?php echo $userdata['avatar']; ?>" alt="Smiley face" width="100">
									</div>
									<br>
									<div>
										<label for="department">Customer: </label> 
										<select name="client_id" class="form-control" style="max-width:50%">
										<?php $sql_query1 = "SELECT * FROM customer_master WHERE customer_id = '".$userdata['client_id']."'";
										      $result1 = mysqli_query($mycon, $sql_query1);
											 if (mysqli_num_rows($result1) > 0) { 
												while ($row1 = mysqli_fetch_assoc($result1)) { ?>
											    <option value="<?php echo $row1['customer_id']; ?>" <?php if($userdata['client_id'] == $row1['customer_id']){ echo "selected"; } ?>><?php echo $row1['customer_code']."-".$row1['customer_name']; ?></option>
											 <?php }   }  ?>
										</select>
									</div>
									<br>
