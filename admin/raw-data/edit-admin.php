                      ...
                      <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
                        ...
                        <div>
                          ...
                          <select name="warehouse_location" class="form-control" style="max-width:50%">
                            <?php
                            $locationquery1 = "SELECT * FROM tbl_location WHERE isDeleted != '1'";
                            $locationResult = mysqli_query($mycon, $locationquery1);
                            if (mysqli_num_rows($locationResult) > 0) {

                              while ($rowlocationResult = mysqli_fetch_assoc($locationResult)) {
                                ?>
                                <option <?php
                                if ($rowlocationResult['loc_id'] == $adminInfo['org_id']) {
                                  echo "selected=selected";
                                }
                                ?> value="<?php echo $rowlocationResult['loc_id']; ?>"><?php echo $rowlocationResult['loc_name']; ?></option>
                                  <?php
                                }
                              }
                              ?>
