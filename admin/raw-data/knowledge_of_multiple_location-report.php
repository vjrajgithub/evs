...
// Merge Columns for showing 'Employee Matrix's Data' close--------------->
  ...
  if (mysqli_num_rows($result_loc) > 0) {
    ...
    while ($rowloc = mysqli_fetch_assoc($result_loc)) {
      ...
      $objPHPExcel->getActiveSheet()->setCellValue($columnFor . $rowCnt, $rowloc['loc_name']);
      $columnFor++;
      //}
    }
  }

  $objPHPExcel->getActiveSheet()->setCellValue($columnFor . $rowCnt, '% of Location Knowledge');
  ...
  $objPHPExcel->getActiveSheet()
          ->getCell('A1')
          ->setValue("Knowledge of multiple location report");
...
//$objPHPExcel->getActiveSheet()->getColumnDimension('A2:X2')->setAutoSize(true);
    ...
    while ($row2 = mysqli_fetch_array($result2)) {
      ...
      if (mysqli_num_rows($result_loc1) > 0) {
        ...
        while ($rowloc1 = mysqli_fetch_assoc($result_loc1)) {
          $sql_query_loc2 = "SELECT * FROM `tbl_emp_dept_matrix` WHERE loc_code = '" . $rowloc1['loc_id'] . "' AND emp_id = '" . $row2['emp_code'] . "'";
  ...
  $file = "knowledge_of_multiple_location-report-" . $dated . ".xls";
