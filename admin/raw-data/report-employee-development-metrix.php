...
// Merge Columns for showing 'Matrix's Data' start---------------
  ...
  $objPHPExcel->getActiveSheet()
          ->getCell('A1')
          ->setValue("The Manpower In The EMPLOYEE DEVELOPMENT MATRIX REPORT");
...
//$objPHPExcel->getActiveSheet()->getColumnDimension('A2:X2')->setAutoSize(true);
  ...
  if ($_POST['item'] != '') {
    ...
    $sql_query = "SELECT tm . *, tu . *  FROM tbl_users tu, tbl_emp_dept_matrix tm WHERE tu.emp_code = tm.emp_id
     " . $role_item . " AND tu.isDeleted != 1 AND tu.roleId IN (2, 3, 4, 13, 14) group by tm.emp_id order by tu.name";
  }
  ...
  $file = "EMPLOYEE-DEVELOPMENT-MATRIX-REPORT-" . $dated . ".xls";
