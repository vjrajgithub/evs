<?php
$dataAbout = "Employee with 100% Knowledge in BINNING SYSTEM PROCESS";
...
function get_total_no_of_rows($mycon, $srcitem, $srcfrom, $role_item) {
  $sql_query = "SELECT tm . *, tu . * FROM tbl_users tu, tbl_emp_dept_matrix tm WHERE tu.emp_code = tm . emp_id AND
          tm . binning_system_process = 100 " . $role_item . " ";
  $result = mysqli_query($mycon, $sql_query);
  return mysqli_num_rows($result);
}
...
<form action="report-binning-system-process.php" onsubmit="submitForm()" method="POST" style="display: flex;
