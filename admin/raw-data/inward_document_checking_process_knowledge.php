<?php
$dataAbout = "Employee with 100% Knowledge in Inward Document Checking process";
...
function get_all_no_of_rows($mycon, $srcitem, $srcfrom, $role_item) {
  $sql_query = "SELECT tm . *, tu . * FROM tbl_users tu, tbl_emp_dept_matrix tm WHERE tu.emp_code = tm.emp_id AND
    tm.inward_document_checking_process = 100
...
function get_total_no_of_rows($mycon, $srcitem, $srcfrom, $role_item) {
  $sql_query = "SELECT tm . *, tu . * FROM tbl_users tu, tbl_emp_dept_matrix tm WHERE tu.emp_code = tm . emp_id AND
          tm . inward_document_checking_process = 100 " . $srcitem . " " . $srcfrom . " " . $role_item . " ";
  $result = mysqli_query($mycon, $sql_query);
  return mysqli_num_rows($result);
}
...
<form action="report-inward-document-checking-process.php" onsubmit="submitForm()" method="POST" style="display: flex;justify-content: center;text-align: center;">
