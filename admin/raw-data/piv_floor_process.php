<?php
$dataAbout = "Employee with 100% Knowledge in PIV FLOOR PROCESS";
...
function get_all_no_of_rows($mycon, $srcitem, $srcfrom, $role_item) {
  $sql_query = "SELECT tm . *, tu . * FROM tbl_users tu, tbl_emp_dept_matrix tm WHERE tu.emp_code = tm.emp_id AND
    tm.piv_floor_process = 100
    " . $srcitem . " " . $srcfrom . " " . $role_item . " AND tu.isDeleted != 1 AND tu.roleId IN (2, 3, 4, 13, 14)
...
<form action="report-piv-floor-process.php" onsubmit="submitForm()" method="POST" style="display: flex;justify-content: center;text-align: center;">
...
if ($_POST['item'] != '') {
  ...
  $sql_query = "SELECT tm . *, tu . * FROM tbl_users tu, tbl_emp_dept_matrix tm WHERE tu.emp_code = tm.emp_id AND
                                                    tm.piv_floor_process = 100
                                                    " . $role_item . " " . $role_item . " AND tu.isDeleted != 1 AND tu.roleId IN (2, 3, 4, 13, 14) group by tm.emp_id order by tm.id LIMIT 0, 20";
}
