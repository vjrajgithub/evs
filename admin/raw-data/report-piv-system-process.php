...
$dataAbout = "EMPLOYEE WITH 100% KNOWLEDGE IN PIV SYSTEM PROCESS";
...
if ($_POST['item'] != '') {
  $sql2 = "SELECT tm . *, tu . * FROM tbl_users tu, tbl_emp_dept_matrix tm WHERE tu.emp_code = tm.emp_id AND
        tm.piv_system_process = 100
        " . $srcitem . " " . $srcfrom . " " . $role_item . " AND tu.isDeleted != 1 AND tu.roleId IN (2, 3, 4, 13, 14) group by tm.emp_id order by tm.id";
} else {
  $sql2 = "SELECT tm . *, tu . * FROM tbl_users tu, tbl_emp_dept_matrix tm WHERE tu.emp_code = tm.emp_id AND
        tm.piv_system_process = 100 " . $role_item . " AND tu.isDeleted != 1 AND tu.roleId IN (2, 3, 4, 13, 14)
...
$file = "Employee-with-100-percent-Knowledge-in-PIV-SYSTEM-PROCESS-" . $dated . ".xls";
